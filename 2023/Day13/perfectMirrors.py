...
def test_horizontal(matrix: np.ndarray) -> int:
    ...
    for index in range(1, max_index):
        ...
        if np.array_equal(matrix[index-num_rows:index], matrix[index:index+num_rows][::-1]):
            return index
    return 0
...
def test_vertical(matrix: np.ndarray) -> int:
    ...
    for index in range(1, max_index):
        ...
        if np.array_equal(matrix[:, index-num_cols:index], np.flip(matrix[:, index:index+num_cols], axis=1)):
            return index
